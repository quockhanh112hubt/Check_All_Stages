...
class Data_P230_Checker_V2:
    def __init__(self, root):
        self.root = root
        self.is_running = False
        self.stage_checker = None
        self.stage_cards = {}
        root.title(f"P230 Production Data Tracker v{get_current_version()}")
    ...
    def _create_header(self, parent):
        ...
        title = tk.Label(title_frame, text="P230 PRODUCTION LINE", 
    ...
    def _create_stages_section(self, parent):
        ...
        progress_label = tk.Label(title_frame, text="18 Stages", 
                                 font=('Segoe UI', 10),
                                 bg=COLORS['white'], fg=COLORS['text_secondary'])
        ...
        stages_config = [
            ("Production Stages", [
                ...
                ("LCD LED", "LCD_LED"), ("Charge Test", "CHARGE"), ("Calibration", "CALI"),
                ("Verification", "VERIFI"), ("Puffing", "PUFFING"), ("Sensor", "SENSOR"),
                ("Inductive", "INDUCTIVE"), ("Cartridge", "CARTRIDGE"), ("SN Writing", "SN_WRITE"),
                ("Final Test", "FINAL"),
            ]),
            ("Packaging Stages", [
    ...
    def _start_background_checks(self, mcu_id):
        stage_configs = [
            ('FIRMWARE', get_data_firmware, 'Firmware'),
            ('PBA', get_data_pba_function, 'PBA Function'),
            ('HEATER', get_data_heater, 'Heater'),
            ('LEAK', get_data_leak, 'Leak Test'),
            ('3QR_MATCH', get_data_matching, '3QR Matching'),
            ('SLEEP', get_data_sleep, 'Sleep'),
            ('LCD_LED', get_data_lcdled, 'LCD LED'),
            ('CHARGE', get_data_charge, 'Charge Test'),
            ('CALI', get_data_cali, 'Calibration'),
            ('VERIFI', get_data_verifi, 'Verification'),
            ('PUFFING', get_data_puffing, 'Puffing'),
            ('SENSOR', get_data_sensor, 'Sensor'),
            ('INDUCTIVE', get_data_inductive, 'Inductive'),
            ('CARTRIDGE', get_data_cartridge, 'Cartridge'),
            ('SN_WRITE', get_data_snwriting, 'SN Writing'),
            ('FINAL', get_data_final, 'Final Test'),
...
def create_gui_P230_v2(create_login_ui, create_gui_P1, create_gui_P4, create_gui_P140):
    def logout():
        root_P230.destroy()
        create_login_ui()
    def switch_to_P1():
        root_P230.destroy()
        create_gui_P1(create_login_ui, create_gui_P230_v2, create_gui_P4, create_gui_P140)
    def switch_to_P4():
        root_P230.destroy()
        create_gui_P4(create_login_ui, create_gui_P1, create_gui_P230_v2, create_gui_P140)
    def switch_to_P140():
        root_P230.destroy()
        create_gui_P140(create_login_ui, create_gui_P1, create_gui_P230_v2, create_gui_P4)

    root_P230 = tk.Tk()
    
    # Modern Status Bar
    status_frame = tk.Frame(root_P230, bg=COLORS['dark'], height=35)
    ...
    ui = Data_P230_Checker_V2(root_P230)
    root_P230.mainloop()
