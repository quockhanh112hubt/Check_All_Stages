    ...
    try:
        ...
        query = """
            ...
            (
                ...
                FROM MIGHTY.ADC_P3_SENSOR_CALIBRATION_HISTORY
                WHERE MCU_ID = :mcu_id
            ) A 
            WHERE A.RNK = 1
        """
...
def get_data_optical_cali_P4(mcu_id):
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        query = """
            ...
            (
                SELECT MCU_ID, TOTAL_JUDGMENT, TRANS_TIME, 
                    RANK() OVER (PARTITION BY MCU_ID ORDER BY TRANS_TIME DESC) AS RNK
                FROM MIGHTY.ADC_P4_OPTICAL_CAL_STICK_DETECTION_TEST_HISTORY
                WHERE MCU_ID = :mcu_id
            ) A 
            WHERE A.RNK = 1
        """
...
def get_data_vibration_P1(mcu_id):
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        query = """
            ...
            (
                SELECT MCU_ID, TOTAL_JUDGMENT, TRANS_TIME, 
                    RANK() OVER (PARTITION BY MCU_ID ORDER BY TRANS_TIME DESC) AS RNK
                FROM MIGHTY.ADC_VIBRATION_HISTORY
                WHERE MCU_ID = :mcu_id
            ) A 
            WHERE A.RNK = 1
        """
