    ...
    try:
        ...
        query = """
            ...
            (
                ...
                FROM MIGHTY.ADC_P3_LCDLED_TEST_HISTORY
                WHERE MCU_ID = :mcu_id
            ) A 
            WHERE A.RNK = 1
        """
...
def get_data_display_P4(mcu_id):
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        query = """
            ...
            (
                SELECT MCU_ID, TOTAL_JUDGMENT, TRANS_TIME, 
                    RANK() OVER (PARTITION BY MCU_ID ORDER BY TRANS_TIME DESC) AS RNK
                FROM MIGHTY.ADC_P4_MMI1_HISTORY
                WHERE MCU_ID = :mcu_id
            ) A 
            WHERE A.RNK = 1
        """
...
def get_data_lcdled_P140(mcu_id):
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        query = """
            ...
            (
                SELECT MCU_ID, TOTAL_JUDGMENT, TRANS_TIME, 
                    RANK() OVER (PARTITION BY MCU_ID ORDER BY TRANS_TIME DESC) AS RNK
                FROM MIGHTY.ADC_P140_LCDLED_TEST_HISTORY
                WHERE MCU_ID = :mcu_id
            ) A 
            WHERE A.RNK = 1
        """
