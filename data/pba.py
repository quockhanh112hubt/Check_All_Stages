...
def get_data_pba_function(mcu_id):
    try:
...
def get_data_pba_function_P4(mcu_id):
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        query = """
...
def get_data_pba_function_P1(mcu_id):
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        query = """
        ...
        (
            SELECT MCU_ID, TOTAL_JUDGMENT, TRANS_TIME, 
                   RANK() OVER (PARTITION BY MCU_ID ORDER BY TRANS_TIME DESC) AS RNK
            FROM MIGHTY.ADC_P1VX_FUNCTION_HISTORY
            WHERE MCU_ID = :mcu_id
        ) A 
        WHERE A.RNK = 1
        """
...
def get_data_pba_function_P140(mcu_id):
    try:
        connection = get_db_connection()
        cursor = connection.cursor()
        query = """
